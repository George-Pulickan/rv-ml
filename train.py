...
def collate_rv(batch):
    ...
    return x, lsp, theta, t_span, t_min, rv_std, valid
...
def _one_epoch(
    ...
    with ctx:
        for x, lsp, theta_target, t_span, t_min, rv_std, valid in loader:
...
def train(
    ...
    if resume and Path(resume).exists():
        ...
        print(f"Resumed from {resume}  (epoch {ckpt.get('epoch', '?')})")
    ...
    if not finetune_only and pretrain_epochs > 0:
        ...
        steps_per_epoch = math.ceil(pretrain_n / batch_size)
...
def _save_ckpt(encoder, opt, epoch, metrics, path):
