...
def train_random_forest_regressor(embedded_csv_path, random_state=42):
    """Train a RandomForest regressor on embedded Fourier features."""
    ...
    model = RandomForestRegressor(n_estimators=5, max_depth=3, random_state=random_state, n_jobs=-1)
    model.fit(X, y)
    return model


def predict_with_random_forest(model, trajectories):
...
def compute_conformal_quantile(predictions, calibration_trajectories, T, dt, theta_0,
    ...
    for pred, traj in zip(predictions, calibration_trajectories):
        ...
        scores.append(abs(float(pred) - matched_l))
...
def build_conformal_prediction_sets(predictions, conformal_radius):
    ...
    lower = np.clip(pred_l - conformal_radius, 0.0, None)
    upper = pred_l + conformal_radius
...
def plot_realistic_prediction_comparison(realistic_trajectories, predictions,
    ...
    ax.scatter(true_l, pred_l, alpha=0.8, color="tab:blue")
    ax.scatter(inferred_l, pred_l, alpha=0.8, color="tab:orange", marker="*", s=80)
    ...
    if prediction_sets is not None:
        ...
        title = f"Length prediction (empirical validity = {empirical_validity:.3f})"
    else:
    ...
    ax.legend([legend_handle], [f"MSE = {mse:.3f}"], loc="upper left", frameon=False)
...
def plot_unseen_trajectories(heldout_trajectories, simulator, n_examples=4, save_path="Data/heldout_synthetic_comparison.png"):
    ...
    for ax, idx in zip(axes, range(n_examples)):
        ...
        ax.set_title(f"Trajectory {traj['id']}\n(l={traj['l']:.3f}, k={traj['k']:.4f})")
        ax.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    rf_model = train_random_forest_regressor(embedded_csv_path, random_state=42)
    realistic_sequences = [traj["theta"] for traj in heldout_trajectories]
    predictions = predict_with_random_forest(rf_model, realistic_sequences)

    _, calibration_trajectories, calibration_labels = load_synthetic_trajectory_data(synthetic_calibration_csv_path)
    calibration_predictions = predict_with_random_forest(rf_model, calibration_trajectories)
