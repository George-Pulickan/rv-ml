...
def base_summary_row(x: np.ndarray, lsp: np.ndarray, theta, info: dict) -> tuple:
    """The targets + observation-summary + coarse-spectral block shared by every
    synthetic regression CSV.

    Single source of truth on purpose: ``generate_lsp_regression_csv`` promises
    that its rows are drawn from the same systems as this module's, so that the
    64-bin vs 512-bin spectral comparison is apples-to-apples.  When this block
    was duplicated in both files, editing a feature definition in one and not
    the other would have silently broken that guarantee with no test to catch
    it.

    Returns ``(row, xm, rv_ms, t_days)`` — the extras are the intermediates the
    callers need for their own additional columns (phase-fold, full LSP).
    """
...
def generate_rows(
    ...
    *,
    ...
    for i in range(n_samples):
        ...
        row, xm, rv_ms, t_days = base_summary_row(x, lsp, theta, info)
        if with_phasefold:
            if fold_period == "lsp":
