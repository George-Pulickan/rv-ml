...
def recompute_phasefold_block(
    ...
    *,
    ...
    for j, (idx, lp) in enumerate(zip(row_indices, log10_P)):
        x, _, _, info = replay_synthetic_sample(
            int(idx), seed, f_multi=f_multi, n_samples=n_samples
        )
...
def _metrics_for_json(metrics: dict) -> dict:
    """Drop numpy arrays and other non-JSON-serializable prediction buffers."""
    out: dict = {}
    for key, val in metrics.items():
        if key in ("y_true", "y_pred"):
            continue
        if isinstance(val, dict):
            out[key] = _metrics_for_json(val)
        elif isinstance(val, np.ndarray):
            continue
        elif isinstance(val, (np.floating, np.integer, np.bool_)):
            out[key] = val.item() if hasattr(val, "item") else bool(val)
        else:
            out[key] = val
    return out
...
def main() -> None:
    ...
    if args.fold_period == "predicted" and args.feature_set in ("35", "109"):
        ...
        metrics["predicted_p_fold_eval"] = _metrics_for_json(gate_c)
    ...
    skip_real_transfer = args.feature_set in ("35", "109")
    if skip_real_transfer:
        print(
            "real transfer skipped for phase-fold feature sets "
            "(catalog systems lack t_peri for 35/109-D inputs)"
        )
        metrics["real_transfer"] = {
            "skipped": True,
            "reason": "no finite phase-fold features on real systems",
            "feature_set": args.feature_set,
        }
    else:
        ...
        if n_excluded:
            print(f"real transfer: excluded {n_excluded} rows with non-finite values")
    ...
    metrics_path.write_text(json.dumps(_metrics_for_json(metrics), indent=2))
    ...
    _print_omega_headline(metrics)
    rt = metrics.get("real_transfer", {})
    if not rt.get("skipped") and args.combined_target in rt.get("per_target", {}):
        t = rt["per_target"][args.combined_target]
        print(f"real transfer ({args.combined_target}): R2={t['r2']:.3f}  MSE={t['mse']:.5f}")
